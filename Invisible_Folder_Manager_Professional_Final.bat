...
rem ============================================================================
rem Invisible Folder Manager - Professional Clean Edition v7.7
rem
...
rem - Self-destruct watchdog deletes only this batch file when its host exits.
rem - Interactive menu actions and noninteractive background helpers use separate launchers.
rem - UAC denial and menu option 9 also trigger deletion of this batch file.
...
if not exist "%IFT_POWERSHELL%" (
    ...
    echo  This file could not arm its self-destruct watchdog.
...
if not exist "%IFT_CHOICE%" (
    ...
    echo  This batch file will now remove itself.
    call :SCHEDULE_SELF_DELETE
...
if errorlevel 1 (
    ...
    "%IFT_POWERSHELL%" -NoLogo -NoProfile -NonInteractive -Command ^
      ...
      "  Start-Process -FilePath $env:IFT_SELF -ArgumentList @('--elevated',$local64,$profile64,$sid) -Verb RunAs -ErrorAction Stop;" ^
      "} catch {" ^
    ...
    if errorlevel 1 (
        ...
        call :SCHEDULE_SELF_DELETE
        if errorlevel 1 (
            echo  WARNING: Self-deletion could not be scheduled.
            echo  Windows or security software may be blocking file deletion.
            echo  Press any key to close.
            pause >nul
        ) else (
            timeout /t 1 /nobreak >nul
...
rem Arm the watchdog before initialization. If this cannot be done, do not run
rem the management functions because the requested close-to-delete behavior
rem would not be active.
call :ARM_SELF_DESTRUCT
if errorlevel 1 (
    echo.
    echo  ERROR: The self-destruct watchdog could not be started.
    echo  The folder manager will not continue without that protection.
    echo  This batch file will now remove itself.
    call :SCHEDULE_SELF_DELETE
    timeout /t 1 /nobreak >nul
    endlocal
...
if errorlevel 1 (
    ...
    echo  Closing now will cause this batch file to delete itself.
    echo  Press any key to close.
    pause >nul
    endlocal
    exit /b 1
...
:ARM_SELF_DESTRUCT
if defined IFT_WATCHDOG_ARMED exit /b 0
set "IFT_ACTION=ArmSelfDestruct"
call :RUN_EMBEDDED_BACKGROUND >nul 2>&1
if errorlevel 1 exit /b 1
set "IFT_WATCHDOG_ARMED=1"
exit /b 0

:SCHEDULE_SELF_DELETE
set "IFT_ACTION=DeleteSelfSoon"
call :RUN_EMBEDDED_BACKGROUND >nul 2>&1
exit /b %errorlevel%

:RUN_EMBEDDED
rem Prompt-capable session for menu/status and all user-facing operations.
rem -Sta keeps the built-in Windows folder picker compatible on Windows 10/11.
...
:RUN_EMBEDDED_BACKGROUND
rem Background watchdog/deletion actions never request console input.
"%IFT_POWERSHELL%" -NoLogo -NoProfile -NonInteractive -Command ^
  "$raw=[IO.File]::ReadAllText($env:IFT_SELF);" ^
  "$marker='# POWERSHELL_SECTION_START';" ^
  "$index=$raw.LastIndexOf($marker,[StringComparison]::Ordinal);" ^
...
:END
cls
title Invisible Folder Manager - Self-Destruct
echo.
echo ==============================================================================
echo   EXITING AND DELETING THIS BATCH FILE
echo ==============================================================================
echo.
echo   Managed folders and their contents will not be deleted.
echo   Only this running .bat file is targeted.
echo.
call :SCHEDULE_SELF_DELETE
if errorlevel 1 (
    echo   WARNING: Self-deletion could not be scheduled.
    echo   Windows or security software may prevent deletion.
    timeout /t 3 /nobreak >nul
) else (
    echo   Self-deletion is scheduled.
    timeout /t 1 /nobreak >nul
)
endlocal
exit /b 0
...
function Get-ParentProcessIdentity {
    ...
    try {
        ...
        # Windows PowerShell 5.1 also provides the older WMI cmdlet.
    }
    ...
    if (($null -eq $parentId) -or ($parentId -le 0)) {
        try {
            ...
            # The caller receives a clear error below.
        }
    }

    if (($null -eq $parentId) -or ($parentId -le 0)) {
        throw 'The batch host process could not be identified.'
    }

    $parentProcess = Get-Process -Id $parentId -ErrorAction Stop

    return [pscustomobject]@{
        Id = [int]$parentProcess.Id
        StartTicks = [Int64]$parentProcess.StartTime.ToUniversalTime().Ticks
    }
}

function Get-SelfDeleteTarget {
    if ([string]::IsNullOrWhiteSpace($env:IFT_SELF)) {
        throw 'The running batch-file path is unavailable.'
    }

    $target = [IO.Path]::GetFullPath($env:IFT_SELF)

    if (-not [IO.File]::Exists($target)) {
        throw "The running batch file no longer exists: $target"
    }

    if ([IO.Directory]::Exists($target)) {
        throw 'The self-delete target unexpectedly resolves to a directory.'
    }

    $extension = [IO.Path]::GetExtension($target)
    if (($extension -ine '.bat') -and ($extension -ine '.cmd')) {
        throw 'The self-delete target is not a .bat or .cmd file.'
    }
...
function Start-SelfDestructWatchdog {
    $target = Get-SelfDeleteTarget
    $hostIdentity = Get-ParentProcessIdentity

    $powerShellPath = $env:IFT_POWERSHELL
    if ([string]::IsNullOrWhiteSpace($powerShellPath) -or
        -not (Test-Path -LiteralPath $powerShellPath -PathType Leaf)) {
        throw 'Windows PowerShell is unavailable for the self-destruct watchdog.'
    }

    $readyFile = Join-Path $env:TEMP (
        'IFT-Watchdog-' + [Guid]::NewGuid().ToString('N') + '.ready'
    )

    $target64 = [Convert]::ToBase64String(
        [Text.Encoding]::Unicode.GetBytes($target)
    )
    $ready64 = [Convert]::ToBase64String(
        [Text.Encoding]::Unicode.GetBytes($readyFile)
    )
...
`$target = [Text.Encoding]::Unicode.GetString(
    [Convert]::FromBase64String('$target64')
)
`$readyFile = [Text.Encoding]::Unicode.GetString(
    [Convert]::FromBase64String('$ready64')
)
`$expectedPid = [int]$($hostIdentity.Id)
`$expectedStartTicks = [Int64]$($hostIdentity.StartTicks)
...
try {
    ...
    `$hostProcess = Get-Process -Id `$expectedPid -ErrorAction Stop
    `$actualStartTicks = [Int64](
...
for (`$attempt = 1; `$attempt -le 120; `$attempt++) {
    try {
        ...
        try {
            ...
            # Remove-Item -Force is attempted below as a fallback.
        }
    ...
    try {
        while ([DateTime]::UtcNow -lt $deadline) {
            ...
            if ($watchdog.HasExited) {
                throw (
                    'The self-destruct watchdog exited before it was armed. ' +
                    "Exit code: $($watchdog.ExitCode)"
                )
            }
        ...
        throw 'The self-destruct watchdog did not confirm startup.'
    }
    catch {
        try {
...
function Start-DelayedSelfDelete {
    $target = Get-SelfDeleteTarget

    $powerShellPath = $env:IFT_POWERSHELL
    if ([string]::IsNullOrWhiteSpace($powerShellPath) -or
        -not (Test-Path -LiteralPath $powerShellPath -PathType Leaf)) {
        throw 'Windows PowerShell is unavailable for delayed self-deletion.'
    }
...
# Give cmd.exe time to return from the current batch file before deletion.
...
"@
    ...
    try {
        ...
        if ($deleteProcess.HasExited -and ($deleteProcess.ExitCode -ne 0)) {
            throw (
                'The delayed self-delete helper exited unexpectedly. ' +
                "Exit code: $($deleteProcess.ExitCode)"
            )
        }
    }
    finally {
...
function Read-ConsoleInput {
    ...
    try {
        ...
        try {
            [Console]::Write(('{0}: ' -f $Prompt))
            $value = [Console]::ReadLine()

            if ($null -eq $value) {
                throw 'The console input stream returned no data.'
            }
            ...
            throw (
                'Interactive console input is unavailable. ' +
                'Run this batch file from a normal Windows console session.'
            )
        }
    }
}
...
function Read-NumericChoice {
    ...
    while ($true) {
        $raw = Read-ConsoleInput -Prompt $Prompt
        $number = 0
    ...
    while ($true) {
        ...
        switch ($method) {
            ...
            2 {
                ...
                $entered = Read-ConsoleInput -Prompt 'Enter the parent folder path'
...
function Convert-VisibleFolder {
    ...
    switch ($selection) {
        ...
        3 {
            Write-Host ''
            $entered = Read-ConsoleInput -Prompt 'Enter the visible folder name or full path'
...
function Show-ProfessionalMenu {
    ...
    Write-MenuOption -Key '9' -Description 'Exit and permanently delete this batch file'
    Write-MenuBorder
    Write-MenuRow `
        ...
        -Align Center
    Write-MenuRow `
        -Text 'Self-destruct armed: closing this window or choosing 9 deletes this .bat.' `
        -Color Yellow `
        -Align Center
    Write-MenuBorder
    Write-Host ''
